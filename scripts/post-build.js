...
// Priority 3: firebase-config.json in project root
if (!firebaseConfig) {
  try {
    const rootConfigPath = path.join(__dirname, '..', 'firebase-config.json');
    if (fs.existsSync(rootConfigPath)) {
      const content = fs.readFileSync(rootConfigPath, 'utf8');
      firebaseConfig = JSON.parse(content);
      console.log('✓ Loaded from firebase-config.json in project root');
    }
  } catch (err) {
    console.warn('⚠  Could not read firebase-config.json:', err.message);
  }
}
...
if (firebaseConfig && firebaseConfig.apiKey) {
  ...
  console.log('\n   To fix: Set VITE_FIREBASE_* environment variables');
  console.log('   OR create firebase-config.json in project root');
}
...
function handleRender(config, configPath) {
  ...
  if (config && config.apiKey) {
    ...
    console.log('   ✓ firebase-config.json written to dist/');
    console.log('   ✓ Server will serve it from /firebase-config.json');
  } else {
    console.log('   ⚠️  No Firebase config found');
    console.log('   → Set VITE_FIREBASE_* environment variables in Render dashboard');
  }
}
...
function handleVercel(config, configPath) {
  ...
  if (config && config.apiKey) {
    fs.writeFileSync(configPath, JSON.stringify(config, null, 2), 'utf8');
    console.log('   ✓ firebase-config.json written to dist/');
    console.log('   ✓ Vercel will serve static files from dist/');
  } else {
    console.log('   ⚠️  No Firebase config found');
    console.log('   → Set VITE_FIREBASE_* environment variables in Vercel dashboard');
  }
}

function handleCPanel(config, configPath) {
  console.log('\n📦 cPanel Configuration:');
  console.log('   Note: cPanel requires manual setup via FTP');
  ...
  if (config && config.apiKey) {
    ...
    console.log('      2. Ensure firebase-config.json is in public_html/ root');
    console.log('      3. Your app should load Firebase correctly');
  } else {
    console.log('   ⚠️  No Firebase config found');
    console.log('\n   📝 Before deploying to cPanel:');
    console.log('      1. Create firebase-config.json with your credentials');
