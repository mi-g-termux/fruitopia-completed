// lib/payments/_paytm/initiate.ts
// Paytm (India) — All-in-One SDK txnToken generator + hosted redirect URL.
// Server-side: prefers ENV vars (PAYTM_MID / PAYTM_MERCHANT_KEY / PAYTM_SANDBOX);
// falls back to values posted from the admin panel for local/dev convenience.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const merchantId  = String(process.env.PAYTM_MID || body.merchantId || '').trim();
    const merchantKey = String(process.env.PAYTM_MERCHANT_KEY || body.merchantKey || '').trim();
    const sandboxMode =
