// lib/payments/_easypaisa/initiate.ts
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const storeId = String(process.env.EASYPAISA_STORE_ID || body.storeId || '').trim();
    const hashKey = String(process.env.EASYPAISA_HASH_KEY || body.hashKey || '').trim();
    const sandboxMode =
