// lib/payments/_payfast/initiate.ts
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const merchantId  = String(process.env.PAYFAST_MERCHANT_ID  || body.merchantId  || '').trim();
    const merchantKey = String(process.env.PAYFAST_MERCHANT_KEY || body.merchantKey || '').trim();
    const passphrase  = String(process.env.PAYFAST_PASSPHRASE   || body.passphrase  || '').trim();
    const sandboxMode =
