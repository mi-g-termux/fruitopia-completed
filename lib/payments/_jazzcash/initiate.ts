// lib/payments/_jazzcash/initiate.ts
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const merchantId    = String(process.env.JAZZCASH_MID      || body.merchantId    || '').trim();
    const password      = String(process.env.JAZZCASH_PASSWORD || body.password      || '').trim();
    const integritySalt = String(process.env.JAZZCASH_SALT     || body.integritySalt || '').trim();
    const sandboxMode =
