...
export const CartModal = ({ isOpen, onClose, emailVerified = true }: CartModalProps) => {
    ...
    try {
      ...
      if (AUTOMATIC_PAYMENT_METHODS.includes(paymentMethod)) {
        ...
        if (paymentMethod === 'PayPal') {
          if (!paymentSettings.paypalEnabled || !paymentSettings.paypalClientId || !paymentSettings.paypalClientSecret) {
  ...
  const runFinalTriggerAPI = async (orderInfo: any, methodLabel: string, txnRef?: string) => {
    try {
      ...
      if (methodLabel.startsWith('Paytm') && paymentSettings.paytmMerchantId) {
        setAutoStep(3);
        const orderId = `QF-${Date.now()}`;
      ...
      if (methodLabel.startsWith('JazzCash') && paymentSettings.jazzCashMerchantId) {
        setAutoStep(3);
        const orderId = `QF-${Date.now()}`;
      ...
      if (methodLabel.startsWith('Easypaisa') && paymentSettings.easypaisaStoreId) {
        setAutoStep(3);
        const orderId = `QF-${Date.now()}`;
      ...
      if (methodLabel.startsWith('PayFast') && paymentSettings.payFastMerchantId) {
        setAutoStep(3);
        const orderId = `QF-${Date.now()}`;
